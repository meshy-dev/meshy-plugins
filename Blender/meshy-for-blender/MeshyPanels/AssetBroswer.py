...
class MeshyModel:
    def __init__(self, json_data):
        ...
        self.author = json_data.get('author', 'Unknown')  # 获取作者名称，若无则为 'Unknown'
...
class MeshyApi:
    ...
    def fetch_model_data(self, page_num=1):
        """Fetch model data from the Meshy API."""
        url = f'https://api.meshy.ai/public/showcases?pageNum={page_num}'
    ...
    def download_thumbnail(self, model):
        ...
        # 修正 user_resource 的使用
        thumbnail_dir = bpy.utils.user_resource('SCRIPTS', path="meshy_thumbnails", create=True)
        thumbnail_path = os.path.join(thumbnail_dir, f"{model.name}.jpeg")
...
class MeshySearchOperator(Operator):
    ...
    def execute(self, context):
        ...
        api = MeshyApi()
        # 默认获取第一页的数据
        api.fetch_model_data(page_num=api.page_num)
        props.search_results.clear()

        for model_name, model in api.models.items():
            props.search_results[model_name] = model
...
class MeshyNextPageOperator(Operator):
    ...
    def execute(self, context):
        props = context.window_manager.meshy_browser
        api = MeshyApi()
        api.fetch_model_data(page_num=props.page_num + 1)
        props.search_results.clear()
...
class MeshyLoadThumbnailsOperator(Operator):
    ...
    def download_thumbnail(self, model):
        ...
        preview_collection['meshy'][model.name] = bpy.utils.previews.new().load(model.name, model.thumbnail_path, 'IMAGE')
...
class MeshyDownloadModelOperator(Operator):
    ...
    def execute(self, context):
        ...
        if selected_model_name in props.search_results:
            ...
            model_path = os.path.join(bpy.app.tempdir, f"{model.name}.glb")  # 假设模型为 .glb 格式
            ...
            if response.status_code == 200:
                ...
                self.report({'INFO'}, f"Model {model.name} downloaded and imported successfully.")
            else:
...
class MeshyAssetBrowserPanel(Panel):
    ...
    def draw(self, context):
        ...
        if props.search_results:
            layout.template_icon_view(context.window_manager, "meshy_results", show_labels=True)
        ...
        if props.search_results:
            ...
            if selected_model_name in props.search_results:
                ...
                # 显示模型名称
                layout.label(text=f"Model Name: {selected_model.name}", icon='OBJECT_DATAMODE')

                # 显示作者名称
                layout.label(text=f"Author: {selected_model.author}", icon='USER')

                # 下载模型按钮
                layout.operator("wm.meshy_download_model", text="Download and Import Model", icon='IMPORT')
...
def list_meshy_results(self, context):
    ...
    if not preview_collection.get('meshy'):
        preview_collection['meshy'] = bpy.utils.previews.new()
    items = []
    for i, (model_name, model) in enumerate(props.search_results.items()):
        if model.thumbnail_path:
            items.append((model_name, model_name, "", preview_collection['meshy'][model_name].icon_id, i))
        else:
            items.append((model_name, model_name, "", "QUESTION", i))
    return items
